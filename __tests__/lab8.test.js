describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    await button.click(); 
    const buttonText = await (await button.getProperty('innerHTML')).jsonValue(); 
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const innerText = await (await page.$('#cart-count')).getProperty('innerText'); 
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){
      let shadowRoot = await prodItems[i].getProperty('shadowRoot'); 
      let buttonText = await(await(await shadowRoot.$('button')).getProperty('innerText')).jsonValue(); 
      expect(buttonText).toBe('Remove from Cart'); 
    }
